# Requirements Index

| ID | Title | Status | File | Depends on |
|----|-------|--------|------|------------|
| REQ-001 | Split repository into `material-core` (tooling) and `material` (content) | DONE | [REQ-001](REQ-001.md) | – |
| REQ-002 | Session backlog: tooling ideas to refine post-split | DROPPED | [REQ-002](REQ-002.md) | REQ-001 |
| REQ-003 | Matrix-driven CI with explicit `projects.yml` manifest | OPEN | [REQ-003](REQ-003.md) | REQ-001 |
| REQ-004 | `matctl course add` / `matctl course remove` — course lifecycle CLI | OPEN | [REQ-004](REQ-004.md) | REQ-001, REQ-003 |
| REQ-005 | Standalone document support — `doc` template and `matctl doc add` / `doc remove` | OPEN | [REQ-005](REQ-005.md) | REQ-003, REQ-004 |

New requirement template: [REQ-xxx.md](REQ-xxx.md)
